...
import { TrendingUp, Zap, Clock, Activity, Lightbulb, ArrowUpRight, Loader2 } from 'lucide-react';
...
export function InsightsFeed() {
  ...
  const btcPrice = prices?.bitcoin?.usd ?? prices?.btc?.usd ?? 0;
  const bestRate = btcPrice ? 'BTC → NGN' : 'Loading...';
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs text-zinc-400 font-medium uppercase tracking-wide">Best Rate Now</p>
          </div>
          <p className="text-lg font-bold font-display">{bestRate}</p>
          <p className="text-sm text-zinc-400 mt-1">
            1 BTC ≈ ₦{(btcPrice * 1581).toLocaleString(undefined, { maximumFractionDigits: 0 })}
          </p>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="rounded-2xl border border-zinc-800 bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 p-5 text-white"
      >
        <div className="flex items-center gap-2 mb-4">
          <div className="h-7 w-7 rounded-lg bg-indigo-500/20 flex items-center justify-center">
            <Zap className="h-4 w-4 text-indigo-400" />
          </div>
          <p className="text-xs text-zinc-400 font-medium uppercase tracking-wide">Your Stats</p>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold text-emerald-400">
                ₦{(stats?.totalFiat ?? 0).toLocaleString()}
              </span>
            </div>
            <div className="flex items-center justify-between">
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <p className="text-xs text-zinc-400 font-medium uppercase tracking-wide">Platform Activity</p>
        </div>
          ...
          <div className="space-y-2">
            {activity.slice(0, 5).map((item: any, i: number) => (
              <div key={i} className="flex items-center gap-2 text-xs">
                ...
                <span className="text-zinc-600 ml-auto">{item.time}</span>
              </div>
            ))}
          </div>
        ) : (
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-xs text-zinc-400 font-medium uppercase tracking-wide">Did You Know?</p>
        </div>
        <motion.p
          key={tipIndex}
